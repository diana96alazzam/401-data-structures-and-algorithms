...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let index = recipe.ingredients[i].indexOf(' ', 3);
    let ing = recipe.ingredients[i].slice((index + 1), recipe.ingredients[i].length)
    result.push(ing)
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let splitted = recipe.ingredients[i].split(' ');
    result.push(`${splitted[2]}${(splitted[3]) ? ' ' + (splitted[3]) : ''}${(splitted[4]) ? ' ' + (splitted[4]) : ''}`)
  }
  return result;
};
